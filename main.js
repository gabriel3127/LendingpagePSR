...
// Inicializar EmailJS
(function() {
    // Credenciais diretas do EmailJS (temporário)
    const publicKey = 'CBpvvLBOJW0FYVHHh';
    emailjs.init(publicKey);
    console.log('✅ EmailJS inicializado com credenciais diretas!');
})();
...
document.getElementById('contactForm').addEventListener('submit', function(e) {
    ...
    function validateFormData(formData) {
        ...
        if (!phoneRegex.test(formData.telefone.replace(/\D/g, ''))) {
            return { valid: false, message: 'Telefone inválido. Use o formato (XX) 9XXXX-XXXX.' };
        }
    ...
    if (!checkRateLimit()) {
        showMessage('Muitas tentativas de envio. Aguarde um momento antes de tentar novamente.', 'error');
        return;
    }
    ...
    // Credenciais diretas do EmailJS
    const serviceId = 'service_1irc3hk';
    const templateId = 'template_0hhugyn';
    ...
    emailjs.send(serviceId, templateId, templateParams)
        ...
        .catch(function(error) {
            ...
            if (error.status === 400) {
                ...
                errorMessage += 'Erro de autenticação do serviço de email.';
            } else if (error.status === 404) {
                errorMessage += 'Serviço de email não encontrado.';
            } else {
                errorMessage += 'Tente novamente em alguns minutos ou entre em contato via WhatsApp.';
            }
            ...
            if (typeof gtag !== 'undefined') {
                gtag('event', 'form_error', {
                    ...
                    'event_label': `email_error_${error.status || 'unknown'}`
                });
            }
        })
        .finally(function() {
